#include "scene/NormalMapScene.hpp"
#include "camera.hpp"
#include "imgui.h"
#include "shader.hpp"
#include "texture.hpp"
...
void NormalMapScene::init()
{
    glEnable(GL_DEPTH_TEST);

    std::filesystem::path current_path = std::filesystem::current_path();
    std::filesystem::path vertexPath = current_path / "res/shaders/normal_map/nm.vs";
    std::filesystem::path fragmentPath = current_path / "res/shaders/normal_map/nm.fs";
    ...
    std::filesystem::path diffuseMapPath = current_path / "res/textures/brickwall.jpg";
    std::filesystem::path normalMapPath = current_path / "res/textures/brickwall_normal.jpg";
...
void NormalMapScene::update(float deltaTime, Camera &camera)
{
...
void NormalMapScene::render()
{
    ...
    else
    {
        ...
        m_model_matrix = glm::mat4(1.0f);
        m_model_matrix = glm::rotate(m_model_matrix, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0)));
...
void NormalMapScene::render_ui()
{
    ImGui::Checkbox("Enable TBN", &m_enable_TBN);
}
...
void NormalMapScene::renderBaseQuad()
{
    if (m_baseQuadVAO == 0)
    {
        // positions
...
void NormalMapScene::renderQuad()
{
    if (m_quadVAO == 0)
    {
        // positions
