...
int main()
{
    ...
    std::filesystem::path debugVexPath = currentPath / "res/shaders/debug.vs";
    std::filesystem::path debugFragPath = currentPath / "res/shaders/debug.fs";
 
    // OpenGL initial state
    Shader shader(debugVexPath.string().c_str(), debugFragPath.string().c_str());
    ...
    std::filesystem::path woodPath = currentPath / "res/textures/wood.png";
    unsigned char *data = stbi_load(woodPath.string().c_str(), &width, &height, &nrComponents, 0);
    if (data)
    {
